## 📚 技术文档

### 数据采集策略

#### 1. Reddit 采集器

**技术方案**: Selenium + Chrome Headless

**实现细节**:
- 使用 `webdriver-manager` 自动管理 ChromeDriver
- 采用用户配置文件持久化登录状态
- 智能滚动检测页面底部
- 反爬虫措施：
  - 禁用自动化控制特征
  - 使用真实 User-Agent
  - 隐藏 `navigator.webdriver` 属性

**采集流程**:
1. 搜索指定关键词
2. 滚动加载更多结果
3. 提取帖子列表（标题、URL、投票数等）
4. 访问详情页获取完整内容
5. 过滤垃圾内容

**关键代码位置**: `backend/src/collectors/reddit.py`

**反爬虫策略**:
- 无头模式运行
- 自定义 User-Agent
- CDP (Chrome DevTools Protocol) 隐藏自动化特征
- 随机等待时间

---

#### 2. YouTube 采集器

**技术方案**: YouTube Data API v3 + youtube-transcript-api

**实现细节**:
- 通过 YouTube Data API v3 搜索视频
- 使用 `youtube-transcript-api` 获取字幕
- 使用web-share进行IP轮询

**采集流程**:
1. 调用 YouTube Data API 搜索视频
2. 提取视频元数据（标题、描述、统计信息）
3. 获取视频字幕/转录文本
4. 组合标题 + 字幕作为分析内容

**代理支持**:
- Webshare 住宅代理（推荐生产环境）
- 通用 HTTP/HTTPS 代理
- 环境变量配置：
  ```bash
  HTTP_PROXY=http://127.0.0.1:7890
  WEBSHARE_PROXY_USERNAME=username
  WEBSHARE_PROXY_PASSWORD=password
  ```

**关键代码位置**: `backend/src/collectors/youtube.py`

---

#### 3. Twitter/X 采集器

**技术方案**: Selenium 爬虫（未登录访问）

**实现细节**:
- 类似 Reddit 的 Selenium 方案
- 需要处理 Twitter 的动态加载
- 反爬虫要求更高

**注意事项**:
- Twitter 爬虫容易失败
- 可能需要登录才能获取完整数据
- 建议降低采集频率避免封禁

**关键代码位置**: `backend/src/collectors/twitter.py`

---

### AI Prompt 设计

#### 1. 情感分析 Prompt

**设计策略**: Few-shot Learning + 链式思维

**核心特点**:
- 8 个精心设计的示例（涵盖积极、消极、中性）
- 包含推理过程（reasoning）引导模型思考
- 明确评分标准（0-100 分制）

**示例 Prompt**:

```
You are a sentiment analysis expert. Analyze the sentiment of social media posts on a 0-100 scale.

Text: This product is absolutely amazing! Best purchase I've ever made. Highly recommend to everyone!
Analysis: Score=95, Label=positive
Reasoning: Strong positive words (amazing, best, highly recommend) with exclamation marks indicate very positive sentiment

Text: Terrible experience. The product broke after one day and customer service was unhelpful.
Analysis: Score=15, Label=negative
Reasoning: Negative words (terrible, broke, unhelpful) with specific complaints indicate extremely negative sentiment

[... 更多示例 ...]

Text: {user_input}
Analysis:
```

**评分标准**:
- 0-20: 极度负面（仇恨、愤怒、厌恶）
- 21-40: 负面（失望、沮丧）
- 41-60: 中性（客观事实、平衡观点）
- 61-80: 正面（满意、认可）
- 81-100: 极度正面（喜爱、兴奋）

**关键代码位置**: `backend/src/ai_analysis/prompts/sentiment_prompts.py`

---

#### 2. 观点聚类 Prompt

**设计策略**: Few-shot + 结构化输出
首先根据上一步的情感分析得分将数据分为三部分（0～40、41～80、81～100），再从每部分分别聚类提取观点。

**目标**: 将大量评论归纳为 3个主要观点

**Prompt 结构**:
```
You are an expert at analyzing public opinion and identifying main discussion themes.

Given the following social media posts about "{keyword}", identify the top 3 main opinion clusters.

Requirements:
1. Each cluster should represent a distinct theme/topic
2. Assign a short label to each cluster
3. Provide a brief summary
4. Count how many posts mention this theme

Example:
Input: Posts about iPhone 16
Output:
1. "Battery Life" - Many users report shorter battery life compared to previous models (15 mentions)
2. "Camera Quality" - Praise for improved low-light performance and new features (12 mentions)
3. "Price Concerns" - Some users find the price increase unjustified (8 mentions)

Now analyze these posts:
{posts}

Output the top 3 opinion clusters in the same format:
```

**关键代码位置**: `backend/src/ai_analysis/prompts/clustering_prompts.py`

---

#### 3. 摘要生成 Prompt

**设计策略**: Map-Reduce 模式处理长文本

**Map 阶段**:
```
Summarize the key points from these social media posts about "{keyword}" in 2-3 sentences.

Posts:
{chunk}

Summary:
```

**Reduce 阶段**:
```
Combine these summaries into a comprehensive 2-3 sentence overview:

{summaries}

Combined Summary:
```

**优化策略**:
- 超过 3000 tokens 自动启用 Map-Reduce
- 分批处理，每批最多 30 条帖子
- 智能采样代表性帖子

**关键代码位置**: `backend/src/ai_analysis/prompts/summarization_prompts.py`

---

### Token 优化策略

#### 1. Map-Reduce 模式

**触发条件**:
- 情感分析：> 4000 tokens
- 聚类：> 4000 tokens
- 摘要：> 3000 tokens

**实现方式**:
1. **Map 阶段**: 分批处理文本，生成中间结果
2. **Reduce 阶段**: 合并中间结果，生成最终输出

**优势**:
- 节省 17-35% Token 成本
- 避免上下文长度限制
- 提高处理稳定性

**关键代码位置**: `backend/src/ai_analysis/utils/map_reduce.py`

---

#### 2. 文本预处理

**策略**:
- 提取关键句（节省 60-80% tokens）
- 截断过长文本
- 移除 HTML 标签和特殊字符
- 过滤垃圾内容（广告、乱码）

**示例**:
```python
from src.ai_analysis.utils import TextPreprocessor

# 提取关键句
key_sentences = TextPreprocessor.extract_key_sentences(text, max_sentences=5)

# 清理和截断
cleaned = TextPreprocessor.clean_for_analysis(text, max_length=1000)
```

---

#### 3. 批量处理

**情感分析批处理**:
- 每批最多 10 条帖子
- 并行调用 API
- 减少网络开销

**成本对比**（100 条帖子，gpt-4o-mini）:

| 版本 | Input Tokens | Output Tokens | 总计 | 成本 |
|------|-------------|---------------|------|------|
| 旧版 | ~12,000 | ~2,800 | ~14,800 | ~$0.023 |
| 新版（直接） | ~10,000 | ~2,500 | ~12,500 | ~$0.019 |
| 新版（Map-Reduce） | ~8,000 | ~2,000 | ~10,000 | ~$0.015 |

**节省**: 17-35%

---

### 常见问题及解决方案

#### 1. Reddit 采集失败

**问题**: Selenium 找不到元素或页面加载超时

**解决方案**:
- 增加等待时间：`time.sleep(5)`
- 检查选择器是否更新（Reddit 经常更新 DOM）
- 使用更宽松的定位策略
- 查看日志：`backend/logs/trendpulse_*.log`

**调试脚本**:
```bash
cd backend
python scripts/diagnose_reddit.py
```

---

#### 2. YouTube 字幕获取失败

**问题**: `TranscriptsDisabled` 或 `NoTranscriptFound` 错误

**原因**:
- 视频未开启字幕
- 地区限制
- 网络问题（中国地区需要代理）

**解决方案**:
1. **配置代理**:
   ```bash
   HTTP_PROXY=http://127.0.0.1:7890
   ```

2. **使用 Webshare 住宅代理**（推荐生产环境）:
   ```bash
   WEBSHARE_PROXY_USERNAME=your_username
   WEBSHARE_PROXY_PASSWORD=your_password
   ```

3. **增加超时时间**:
   ```python
   # 在 youtube.py 中
   self.timeout = aiohttp.ClientTimeout(total=120)
   ```

---

#### 3. AI 分析成本过高

**问题**: Token 消耗超出预算

**解决方案**:
1. **使用更便宜的模型**:
   ```bash
   # 使用通义千问（中文更优，价格更低）
   LLM_PROVIDER=tongyi
   TONGYI_MODEL=qwen-plus
   ```

2. **启用 Map-Reduce**:
   ```python
   pipeline = AnalysisPipelineV2(use_map_reduce=True)
   ```

3. **跳过不必要的分析**:
   ```python
   result = await pipeline.analyze_posts(posts, options={
       "skip_clustering": True  # 跳过聚类
   })
   ```

4. **采样代表性帖子**:
   ```python
   sampled_posts = posts[:30]  # 只分析前 30 条
   ```

---

#### 4. 数据库锁定错误

**问题**: `sqlite3.OperationalError: database is locked`

**原因**: SQLite 不支持高并发写操作

**解决方案**:
1. **使用连接池**:
   ```python
   # 在 operations.py 中配置
   engine = create_engine(
       DATABASE_URL,
       pool_size=5,
       max_overflow=10
   )
   ```

2. **生产环境迁移到 PostgreSQL**:
   ```bash
   # 修改 DATABASE_URL
   DATABASE_URL=postgresql+asyncpg://user:pass@localhost/trendpulse
   ```

---

#### 5. Youtube 爬虫被封禁

**问题**: 账号被封或 IP 被限制

**解决方案**:
1. **使用web-share**:
   通过[web-share](https://www.webshare.io/)获取代理进行轮训